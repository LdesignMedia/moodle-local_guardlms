...
$string['settings:baseurl_desc'] = 'Base URL of your GuardLMS instance, for example https://dashboard.guardlms.com. Leave the default unless you run a self-hosted GuardLMS.';
...
$string['settings:sendconfig_desc'] = 'Optional. Also send the site security, HTTP security, session, authentication, web service and debugging settings, plus fixed verdicts from recognised Moodle security checks and aggregate hardening observations, so GuardLMS can audit how the site is configured. Configured cURL IP addresses and CIDR rules may be included for security-policy review. No user records, token values, enrolment keys, user IP histories, absolute paths or report prose are sent by this configuration inventory. Secret settings are represented only by whether they are set. Off by default.';
